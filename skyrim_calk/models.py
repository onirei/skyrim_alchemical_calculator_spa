...
class NameForm(forms.Form):
    ...
    choice_field_last = forms.ChoiceField(label='Ключевой эффект', widget=forms.RadioSelect, choices=CHOICES2, initial='0')
    ...
    def finder (attribute_1, attribute_2, attribute_3, attribute_4):
        ...
        bd_result = NameForm.connection_to_bd(attributes)
        return bd_result
    ...
    def optimizer (attribute_1, attribute_2, attribute_3, attribute_4, bd_data):
        ...
        for ingredient in bd_result:
            ...
            if len(attributes_optima) == 1:
                bd_result_optima += NameForm.connection_to_bd(attributes_optima)
            elif len(attributes_optima) == 2:
                bd_result_optima += NameForm.connection_to_bd(attributes_optima)
                bd_result_optima += NameForm.connection_to_bd((attributes_optima[0],))
                bd_result_optima += NameForm.connection_to_bd((attributes_optima[1],))
            elif len(attributes_optima) == 3:
                bd_result_optima += NameForm.connection_to_bd(attributes_optima)
                bd_result_optima += NameForm.connection_to_bd((attributes_optima[0],))
                bd_result_optima += NameForm.connection_to_bd((attributes_optima[1],))
                bd_result_optima += NameForm.connection_to_bd((attributes_optima[2],))
                bd_result_optima += NameForm.connection_to_bd(attributes_optima[0:1])
                bd_result_optima += NameForm.connection_to_bd(attributes_optima[1:2])
                bd_result_optima += NameForm.connection_to_bd(attributes_optima[1:3:2])
            else:
                pass
            bd_result_optima += (('', '', '', '', '', '',),)
